  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // If 'order' index doesn't exist, this might fail, so we fallback to basic fetch if needed
        let facultyList = [];
        try {
          ...
          <div style={styles.list}>
            {recentFaculty.length === 0 ? <p style={{ color: '#888', fontStyle: 'italic' }}>No faculty members found.</p> : recentFaculty.map(member => (
              <div key={member.id} style={styles.listItem}>
                ...
                <div>
                  <h4 style={styles.itemTitle}>{member.name}</h4>
                  <small style={{ color: '#64748b' }}>{member.role}</small> <br/>
                  <small style={{ color: '#94a3b8', fontSize: '11px' }}>{member.department?.toUpperCase()}</small>
                </div>
              </div>
            ))}
          </div>
...
const styles = {
  ...
  itemTitle: { margin: '0 0 3px 0', fontSize: '15px', color: '#334155' },
